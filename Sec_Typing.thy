...
code_pred (expected_modes: i => i => bool) sec_type . (* que? *)

value "0 \<turnstile> IF Less (V 1) (V 0) THEN 1 ::= N 0 ELSE SKIP"
value "1 \<turnstile> IF Less (V 1) (V 0) THEN 1 ::= N 0 ELSE SKIP"
value "2 \<turnstile> IF Less (V 1) (V 0) THEN 1 ::= N 0 ELSE SKIP"
...
next
  ...
  proof cases
    ...
    with IfTrue.hyps(3) IfTrue.prems(1,3) `sec b \<turnstile> c1`  anti_mono
    show ?thesis by auto
  next
...
next
  ...
  proof cases
    ...
    with IfFalse.hyps(3) IfFalse.prems(1,3) `sec b \<turnstile> c2` anti_mono
    show ?thesis by auto
  next
    assume "\<not> sec b \<le> l"
    have 1: "sec b \<turnstile> IF b THEN c1 ELSE c2"
...
next
  ...
  proof cases
    ...
    from WhileTrue.hyps(5)[OF `(?w,t2) \<Rightarrow> t3` `0 \<turnstile> ?w`
      WhileTrue.hyps(3)[OF `(c,t1) \<Rightarrow> t2` anti_mono[OF `sec b \<turnstile> c`]
        `s1 = t1 (\<le> l)`]]
    show ?thesis by simp
  next
    assume "\<not> sec b \<le> l"
    have 1: "sec b \<turnstile> ?w" by(rule sec_type.intros)(simp_all add: `sec b \<turnstile> c`)
    from confinement[OF big_step.WhileTrue[OF WhileTrue(1,2,4)] 1] `\<not> sec b \<le> l`
